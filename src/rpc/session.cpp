...
void Session::do_read()
{
	...
	http::async_read(socket_, buffer_, req_, boost::asio::bind_executor(strand_, std::bind(&Session::on_read, shared_from_this(), std::placeholders::_1, std::placeholders::_2)));
}
...
void Session::handle_request(boost::beast::string_view doc_root,
	...
	Send&& send)
{
	...
	{
		blockmirror::chain::TransactionPtr transaction = std::make_shared<blockmirror::chain::Transaction>();
	...
	{
		blockmirror::chain::DataPtr data = std::make_shared<blockmirror::chain::Data>();
